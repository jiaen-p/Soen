...
export class ChatService {
  private url = 'http://localhost:4000/conversations';
  ...
  sendMessage(mensaje: string, conversation_id: number){
    const message = new Mensajes();
    message.mensaje = mensaje;
    message.user_id = this.usuario.user_id;
    return this.http.put(this.url, {mensaje: message, conversation_id});
  }
  deleteConversation(conversation_id: number){
    return this.http.request('delete', this.url, {body: {conversation_id}}).toPromise();
  }
  getProjectOwner(project_id: number){
    return this.http.post(this.url + '/project', {project_id}).toPromise();
  }
